  ...
  const renderDates = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      dates.push(
        ...
        >
          <span className={`text-[0.75rem] md:text-sm font-bold mb-1.5 md:mb-2 w-7 h-7 flex items-center justify-center rounded-full ${isToday ? 'bg-gold text-dark' : 'text-dark dark:text-cream'}`}>
            {day}
          </span>
  ...
  return (
    <div className="mb-8">
      <div className="grid grid-cols-7 gap-[1px] bg-dark/10 dark:bg-cream/10 border border-dark/10 dark:border-cream/10 rounded-xl overflow-hidden shadow-sm">
        ...
        {weekDays.map((d) => (
          <div key={d} className="bg-white/80 dark:bg-black/40 py-3 text-center font-bold text-dark/70 dark:text-cream/70 text-xs md:text-sm uppercase tracking-wider">
            {d}
          </div>
        ))}
        ...
        <div className="flex flex-wrap gap-4 md:gap-6">
          {Object.entries(eventTypes).map(([key, value]) => (
            <div key={key} className="flex items-center gap-2">
              ...
              ></span>
              <span className="text-sm font-semibold text-dark/80 dark:text-cream/80">{value.label}</span>
            </div>
          ))}
        </div>
      </div>
